...
def publish_offer(
    ...
    if extracted_text.startswith("Variable reward: up to stated amount."):
        full_description = f"Variable reward: up to stated amount. {check.text[:4900]}"
    else:
        full_description = check.text[:5000]
    ...
    payload = {
        ...
        "audience_tags": [],
        "eligibility_details": candidate.get("eligibility_details"),
...
def deactivate_offer_by_url(
    ...
    response = requests.patch(
        ...
        json={
            ...
            "full_description": f"Offer deactivated during verification: {reason}",
        },
        timeout=TIMEOUT,
    )
    response.raise_for_status()
